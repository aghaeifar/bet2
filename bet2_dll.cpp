...
Option<bool> verbose (string ("-v,--verbose"), false,
                      string ("switch on diagnostic messages"), false,
                      no_argument);
Option<bool>
    generate_mesh (string ("-e,--mesh"), false,
                   string ("generates brain surface as mesh in vtk format"),
                   false, no_argument);
Option<bool> help (string ("-h,--help"), false,
                   string ("displays this help, then exits"), false,
                   no_argument);
Option<bool> outline (
    string ("-o,--outline"), false,
    string ("generate brain surface outline overlaid onto original image"),
    false, no_argument);
Option<bool> skull (string ("-s,--skull"), false,
                    string ("generate approximate skull image"), false,
                    no_argument);
...
Option<bool> no_output (string ("-n,--nooutput"), false,
                        ...
                        false, no_argument);
Option<bool> apply_thresholding (
    string ("-t,--threshold"), false,
    string ("-apply thresholding to segmented brain image and mask"), false,
    no_argument);
Option<float> fractional_threshold (
    string ("-f"), 0.5,
    string ("~<f>\t\tfractional intensity threshold (0->1); default=0.5; "
            "smaller values give larger brain outline estimates"),
    false, requires_argument);

Option<float> gradient_threshold (
    string ("-g"), 0.0,
    string ("~<g>\t\tvertical gradient in fractional intensity threshold "
            "(-1->1); default=0; positive values give larger brain outline at "
            "bottom, smaller at top"),
    false, requires_argument);

Option<float> smootharg (
    string ("-w,--smooth"), 1.0,
    string ("~<r>\tsmoothness factor; default=1; values smaller than 1 "
            "produce more detailed brain surface, values larger than one "
            "produce smoother, less detailed surface"),
    false, requires_argument);

Option<float> radiusarg (string ("-r,--radius"), 0.0,
                         string ("~<r>\thead radius (mm not voxels); initial "
                                 "surface sphere is set to half of this"),
                         false, requires_argument);

Option<float> centerarg (string ("-c"), 0.0,
                         string ("~<x y z>\tcentre-of-gravity (voxels not mm) "
                                 "of initial mesh surface."),
                         false, requires_3_arguments);

void
draw_segment (volume<short> &image, const Pt &p1, const Pt &p2)
{
  ...
  for (double i = 0; i <= d; i += mininc)
    {
      Pt p = p2 + i * n;
      image ((int)floor ((p.X) / xdim + .5), (int)floor ((p.Y) / ydim + .5),
             (int)floor ((p.Z) / zdim + .5))
          = 0;
    }
}

volume<short>
draw_mesh (const volume<short> &image, const Mesh &m)
{
  double xdim = (double)image.xdim ();
  double ydim = (double)image.ydim ();
  double zdim = (double)image.zdim ();
  ...
  for (list<Triangle *>::const_iterator i = m._triangles.begin ();
       i != m._triangles.end (); i++)
    {
...
void
draw_segment_bis (volume<float> &image, const Pt &p1, const Pt &p2, double max)
{
  double xdim = (double)image.xdim ();
  double ydim = (double)image.ydim ();
  double zdim = (double)image.zdim ();
  ...
  for (double i = 0; i <= d; i += mininc)
    {
      Pt p = p2 + i * n;
      image ((int)((p.X) / xdim + .5), (int)((p.Y) / ydim + .5),
             (int)((p.Z) / zdim + .5))
          = max;
    }
}
...
volume<float>
draw_mesh_bis (const volume<float> &image, const Mesh &m)
{
  double xdim = (double)image.xdim ();
  double ydim = (double)image.ydim ();
  double zdim = (double)image.zdim ();
...
// calculates the mask from the mesh by spreading an initial point outside the
// mesh, and stopping it when the mesh is reached.
volume<short>
make_mask_from_mesh (const volume<float> &image, const Mesh &m)
{
  ...
  current.clear ();
  Pt c (0., 0., 0.);
  for (vector<Mpoint *>::const_iterator it = m._points.begin ();
       it != m._points.end (); it++)
    c += (*it)->get_coord ();
  ...
  while (!current.empty ())
    {
      ...
      current.pop_back ();
      mask.value (x, y, z) = 0;
      if (0 <= x - 1 && mask.value (x - 1, y, z) == 1)
        current.push_back (Pt (x - 1, y, z));
      if (0 <= y - 1 && mask.value (x, y - 1, z) == 1)
        current.push_back (Pt (x, y - 1, z));
      if (0 <= z - 1 && mask.value (x, y, z - 1) == 1)
        current.push_back (Pt (x, y, z - 1));
      if (xsize > x + 1 && mask.value (x + 1, y, z) == 1)
        current.push_back (Pt (x + 1, y, z));
      if (ysize > y + 1 && mask.value (x, y + 1, z) == 1)
        current.push_back (Pt (x, y + 1, z));
      if (zsize > z + 1 && mask.value (x, y, z + 1) == 1)
        current.push_back (Pt (x, y, z + 1));
    }
...
volume<float>
inside_mesh (const volume<float> &image, const Mesh &m)
{
  volume<float> res = image;
  ...
  for (int k = 0; k < zsize; k++)
    for (int j = 0; j < ysize; j++)
      for (int i = 0; i < xsize; i++)
        res.value (i, j, k)
            = (1 - inside.value (i, j, k)) * image.value (i, j, k);
  return res;
}

bet_parameters
adjust_initial_mesh (const volume<float> &image, Mesh &m,
                     const double &rad = 0., const double xpara = 0.,
                     const double ypara = 0., const double zpara = 0.)
{
  bet_parameters bp;
  ...
  if (xpara == 0. & ypara == 0. & zpara == 0.)
    ...
    center = Pt (xpara * xdim, ypara * ydim, zpara * zdim);
      ...
      for (int i = 0; i < image.xsize (); i++)
        {
          ...
          if (d > t2 && d < t98
              && ((p - center) | (p - center)) < bp.radius * bp.radius)
            vm.push_back (d);
        }
...
double
step_of_computation (const volume<float> &image, Mesh &m,
                     const double bet_main_parameter, const int pass,
                     const double increase_smoothing,
                     const int iteration_number, double &l, const double t2,
                     const double tm, const double t, const double E,
                     const double F, const double zcog, const double radius,
                     const double local_th = 0., const int d1 = 7,
                     const int d2 = 3)
{
  double xdim = image.xdim ();
  double ydim = image.ydim ();
  ...
  if (iteration_number == 50 || iteration_number % 100 == 0)
    {
      l = 0;
      int counter = 0;
      for (vector<Mpoint *>::iterator i = m._points.begin ();
           i != m._points.end (); i++)
        {
          counter++;
       ...
       i != m._points.end (); i++)
    {
      ...
      // main term of bet
      {
        ...
        if (local_th != 0.0)
          {
            local_t = Min (
                1., Max (0., bet_main_parameter
                                 + local_th * ((*i)->get_coord ().Z - zcog)
                                       / radius));
          }
            ...
            if (image.in_bounds (i2, j2, k2))
              {
                ...
                for (double gi = 2.0; gi < d1; gi += dscale)
                  {
                    // cout << gi << " " << endl;
                    //  the following is a quick calc of Pt p =
                    //  (*i)->get_coord() + (-gi)*n;
                    iv -= nxv;
                    jv -= nyv;
                    kv -= nzv;
      ...
      // cout<<"l "<<l<<"u1 "<<((u1*n).norm())<<"u2 "<<(u2|n)<<"u3
      // "<<(u3|n)<<endl;
...
volume<float>
find_skull (volume<float> &image, const Mesh &m, const double t2, double t,
            double t98)
{
  ...
  for (vector<Mpoint *>::const_iterator i = m._points.begin ();
       i != m._points.end (); i++)
    {
      ...
      for (list<Mpoint *>::const_iterator nei = (*i)->_neighbours.begin ();
           nei != (*i)->_neighbours.end (); nei++)
        max_neighbour = Max (((**i) - (**nei)).norm (), max_neighbour);

      max_neighbour = ceil ((max_neighbour) / 2);

      const Pt mpoint ((*i)->get_coord ().X / xdim,
                       (*i)->get_coord ().Y / ydim,
                       (*i)->get_coord ().Z / zdim);
      for (int ck = (int)floor (mpoint.Z - max_neighbour / zdim);
           ck <= (int)floor (mpoint.Z + max_neighbour / zdim); ck++)
        for (int cj = (int)floor (mpoint.Y - max_neighbour / ydim);
             cj <= (int)floor (mpoint.Y + max_neighbour / ydim); cj++)
          for (int ci = (int)floor (mpoint.X - max_neighbour / xdim);
               ci <= (int)floor (mpoint.X + max_neighbour / xdim); ci++)
            {
              bool compute = false;
              ...
              if (volmesh (ci, cj, ck) == 0)
                {
                  ...
                  for (list<Mpoint *>::const_iterator nei
                       = (*i)->_neighbours.begin ();
                       nei != (*i)->_neighbours.end (); nei++)
                    mindist = Min (((realpoint) - (**nei)).norm (), mindist);
                  if (mindist >= ((realpoint) - (**i)).norm ())
                    compute = true;
                }
              ...
              if (compute)
                {
                  double maxval = t;
                  double minval
                      = image.interpolate (point.X, point.Y, point.Z);
                  double d_max = 0;

                  for (double d = 0; d < skull_search; d += scale * .5)
                    {
                      Pt current = point + d * n;
                      double val = image.interpolate (current.X, current.Y,
                                                      current.Z);
                      if (val > maxval)
                        {
                          maxval = val;
                  ...
                  if (maxval > t)
                    {
                      ...
                      for (double d = skull_start; d < d_max; d += scale * 0.5)
                        {
                          ...
                          if (current.X >= 0 && current.Y >= 0
                              && current.Z >= 0 && current.X < xsize
                              && current.Y < ysize && current.Z < zsize)
                            {
                              double tmpf
                                  = d / 30
                                    - image.interpolate (current.X, current.Y,
                                                         current.Z)
                                          / (t98 - t2);
                              if (tmpf > maxJ)
                                {
                      ...
                      if (current2.X >= 0 && current2.Y >= 0 && current2.Z >= 0
                          && current2.X < xsize && current2.Y < ysize
                          && current2.Z < zsize)
                        {
                          double val2 = image.interpolate (
                              current2.X, current2.Y, current2.Z);
                          for (double d = d_min + scale; d < d_max;
                               d += 0.5 * scale)
                            {
                              Pt current = point + d * n;
                              if (current.X >= 0 && current.Y >= 0
                      ...
                      if (maxgrad > 0)
                        {
                          ...
                          if (current3.X >= 0 && current3.Y >= 0
                              && current3.Z >= 0 && current3.X < xsize
                              && current3.Y < ysize && current3.Z < zsize)
...
{
  int
  runBET (float *BrainData, float *Mask, int NX, int NY, int NZ)
  {
    ...
    if (testvol.xsize () * testvol.xdim () < 20)
      testvol.setxdim (200);
    if (testvol.ysize () * testvol.ydim () < 20)
      testvol.setydim (200);
    if (testvol.zsize () * testvol.zdim () < 20)
      testvol.setzdim (200);

    Mesh m;
    make_mesh_from_icosa (5, m);

    bet_parameters bp = adjust_initial_mesh (testvol, m, radiusarg.value (),
                                             xarg, yarg, zarg);
    ...
    for (int i = 0; i < nb_iter; i++)
      {
        step_of_computation (testvol, m, bet_main_parameter, 0, 0, i, l, bp.t2,
                             bp.tm, bp.t, E, F, bp.cog.Z, bp.radius,
    ...
    if (verbose.value () && !generate_mesh.value ())
      cout << "self-intersection total " << tmp << " (threshold=4000.0) "
           << endl;
    ...
    while (self_intersection)
      {
        if (self_intersection && verbose.value ())
          {
            cout << "thus will rerun with higher smoothness constraint"
                 << endl;
          };
        m = moriginal;
        l = 0;
        ...
        for (int i = 0; i < nb_iter; i++)
          {
            ...
            if (i > .75 * (double)nb_iter)
              incfactor
                  = 4. * (1. - i / (double)nb_iter) * (incfactor - 1.) + 1.;
            step_of_computation (testvol, m, bet_main_parameter, pass,
                                 incfactor, i, l, bp.t2, bp.tm, bp.t, E, F,
                                 bp.cog.Z, bp.radius,
                                 gradient_threshold.value ());
          }
        double tmp = m.self_intersection (moriginal);
    ...
    if (apply_thresholding.value ())
      {
        ...
        for (int k = 0; k < zsize; k++)
          for (int j = 0; j < ysize; j++)
            for (int i = 0; i < xsize; i++)
              if (testvol.value (i, j, k) < bp.t)
                brainmask.value (i, j, k) = 1;
      }
    ...
    if (!(no_output.value ()))
      {
        ...
        for (int k = 0; k < zsize; k++)
          for (int j = 0; j < ysize; j++)
            for (int i = 0; i < xsize; i++)
              output.value (i, j, k)
                  = (1 - brainmask.value (i, j, k)) * output.value (i, j, k);
    ...
    if (generate_mask.value ().size () > 0)
      {
        // if (save_volume((short)1 - brainmask,
        // generate_mask.value().c_str())<0)  return -1;
      }
    else
      {
        string maskstr = out + "_mask";
        // if (save_volume((short)1 - brainmask, maskstr.c_str())<0)  return
        // -1;
    ...
    if (skull.value ())
      {
        ...
        if (save_volume (bskull, skullstr.c_str ()) < 0)
          return -1;
      }
